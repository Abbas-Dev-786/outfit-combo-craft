...
const OutfitZone: React.FC<OutfitZoneProps> = ({ outfit, setOutfit }) => {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div
        ref={drop}
        className={`outfit-zone transition-all duration-300 ${isOver ? "border-primary border-4 scale-[1.02]" : ""}`}
      >
        <h2 className="text-lg font-semibold mb-4 flex items-center">
          Your Outfit
          {score > 80 && <Sparkles className="h-4 w-4 text-yellow-500 ml-2 animate-pulse-gentle" />}
        </h2>
        {outfit.length === 0 ? (
          ...
          <div className="flex flex-wrap gap-4 justify-center">
            {outfit.map((item) => (
              <div key={item.id} className="relative">
                <div 
                  className={`clothing-item animate-scale-in ${getBgColor(item.type)}`}
                >
                  {item.image && (
                  ...
                  <div className="text-xs font-medium truncate relative z-10 bg-white/70 px-2 py-1 rounded-md shadow-sm">{item.name}</div>
                  <div 
      ...
      <Card className="p-4 shadow-md border border-fashion-lavender/30">
        <div className="mb-3 flex justify-between items-center">
          ...
          <Badge variant={score > 80 ? "default" : score > 60 ? "secondary" : "outline"} className="animate-float">
            {score}/100
          </Badge>
        </div>
